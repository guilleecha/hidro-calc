...
from calculators.utils.constants import RUNOFF_COEFFICIENTS
...
def calculate_weighted_C(
    areas_coefficients: List[tuple[float, float]]
) -> float:
    """
    Calcula el coeficiente de escorrentía ponderado para múltiples superficies.
    ...
    if not areas_coefficients:
        raise ValueError(
            "Debe proporcionar al menos un área con su coeficiente"
        )
    ...
    for area_ha, C in areas_coefficients:
        if area_ha <= 0:
            raise ValueError(
                f"El área debe ser positiva. Valor: {area_ha} ha"
            )

        if C < 0 or C > 1:
            raise ValueError(
                f"El coeficiente C debe estar entre 0 y 1. Valor: {C}"
            )
...
def get_runoff_coefficients() -> Dict[str, Dict[str, Any]]:
    """
    Retorna los coeficientes de escorrentía de referencia.
