...
if not DATABASE_URL:
    ...
    print(f"⚠️  No se encontró DATABASE_URL, usando SQLite: {DATABASE_URL}")

print(f"📊 Conectando a: {DATABASE_URL.split('@')[0] if '@' in DATABASE_URL else DATABASE_URL[:50]}")
...
def seed_db():
    ...
    try:
        ...
        if db.query(Project).count() > 0:
            print("⏭️  Base de datos ya contiene datos, saltando seed")
            return
        ...
        db.commit()
        print(f"✅ Seed completado: 1 proyecto, {len(watersheds)} cuencas, {len(storms)} tormentas")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "init":
            ...
            confirm = input("⚠️  ¿Estás seguro? Esto eliminará TODOS los datos. (y/n): ")
            if confirm.lower() == 'y':
                drop_all()
        elif command == "seed":
            init_db()
            ...
            confirm = input("⚠️  Resetear BD (eliminar + crear + seed)? (y/n): ")
            if confirm.lower() == 'y':
                drop_all()
                init_db()
                seed_db()
